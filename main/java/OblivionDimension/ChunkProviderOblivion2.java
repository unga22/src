...
public class ChunkProviderOblivion2 implements IChunkProvider
{
    /** RNG. */
    private Random rand;
    ...
    /** A NoiseGeneratorOctaves used in generating terrain */
    private NoiseGenerator noiseGen4;
    ...
    public NoiseGenerator mobSpawnerNoise;
    ...
    private MapGenBase caveGenerator = new MapGenBase(); //MAPPING GENERACION DE CUEVAS
	...
	private Object scatteredFeatureGenerator;
    ...
    public ChunkProviderOblivion2(World par1World, long par2, boolean par4)
    {
                            ...
                            for (int n = 0; n < 4; ++n)
                            {
                                if ((d16 += d15) > 0.0D)
                                {
                                	if(index + dy >= par3ArrayOfByte.length)
                                		continue;
                          //          par3ArrayOfByte[index += dy] = Blocks.stone;
                                }
                                else if (y1 * 8 + y2 < b2)
                                {
                                	if(index + dy >= par3ArrayOfByte.length)
    ...
    public void replaceBlocksForBiome(int par1, int par2, short[] par3ArrayOfByte, BiomeGenBase[] par4ArrayOfBiomeGenBase)
    {
        ...
        byte b0 = 63;
        double d0 = 0.03125D;
        this.stoneNoise = ((NoiseGeneratorOctaves) this.noiseGen4).generateNoiseOctaves(this.stoneNoise, par1 * 16, par2 * 16, 0, 16, 16, 1, d0 * 2.0D, d0 * 2.0D, d0 * 2.0D);
        ...
        for (int z = 0; z < 16; ++z)
        {
            for (int x = 0; x < 16; ++x)
            {
                ...
                float f = biomegenbase.getFloatTemperature(x, par1, z); //revisar
                int i1 = (int)(this.stoneNoise[z + x * 16] / 3.0D + 3.0D + this.rand.nextDouble() * 0.25D);
                int j1 = -1;
                Block b1 = ((BiomeGenOblivionDesert)biomegenbase).sTopBlock;
                Block b2 = ((BiomeGenOblivionDesert)biomegenbase).sFillerBlock;
                    ...
                    else
                    {
                        ...
                        //else if (b3 == Blocks.stone)
                        {
                            if (j1 == -1)
                            {
                                ...
                                {
                                    b1 = ((BiomeGenOblivionDesert)biomegenbase).sTopBlock;
                                    b2 = ((BiomeGenOblivionDesert)biomegenbase).sFillerBlock;
                                }
    ...
    private double[] initializeNoiseField(double[] par1ArrayOfDouble, int par2, int par3, int par4, int par5, int par6, int par7)
    {
       ...
       // if (event.getResult() == Result.DENY) return event.noisefield;
        ...
        double d0 = 684.412D;
        double d1 = 684.412D;
        this.noise5 = ((NoiseGeneratorOctaves) this.noiseGen5).generateNoiseOctaves(this.noise5, par2, par4, par5, par7, 1.121D, 1.121D, 0.5D);
        this.noise6 = ((NoiseGeneratorOctaves) this.noiseGen6).generateNoiseOctaves(this.noise6, par2, par4, par5, par7, 200.0D, 200.0D, 0.5D);
        this.noise3 = ((NoiseGeneratorOctaves) this.noiseGen3).generateNoiseOctaves(this.noise3, par2, par3, par4, par5, par6, par7, d0 / 80.0D, d1 / 160.0D, d0 / 80.0D);
        this.noise1 = ((NoiseGeneratorOctaves) this.noiseGen1).generateNoiseOctaves(this.noise1, par2, par3, par4, par5, par6, par7, d0, d1, d0);
        this.noise2 = ((NoiseGeneratorOctaves) this.noiseGen2).generateNoiseOctaves(this.noise2, par2, par3, par4, par5, par6, par7, d0, d1, d0);
        boolean flag = false;
        boolean flag1 = false;
        int i2 = 0;
        int j2 = 0;
    ...
    public List getPossibleCreatures(EnumCreatureType par1EnumCreatureType, int par2, int par3, int par4)
    {
       ...
       // return biomegenbase == null ? null : (biomegenbase == BiomeGenBase.swampland && par1EnumCreatureType == EnumCreatureType.monster && ((MapGenStructure) this.scatteredFeatureGenerator).hasStructureAt(par2, par3, par4) biomegenbase.getSpawnableList(par1EnumCreatureType));
    }
