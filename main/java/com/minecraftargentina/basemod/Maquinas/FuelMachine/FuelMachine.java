...
public class FuelMachine extends BlockContainer {
	...
	public FuelMachine(boolean blockState) {
		super(Material.iron);
		rand = new Random();
		isActive = blockState;
    ...
    public boolean onBlockActivated(World world, int x, int y, int z, EntityPlayer player, int side, float hitX, float hitY, float hitZ) {
    	FMLNetworkHandler.openGui(player, BaseMod.instance, CreacionDeMaquinas.guiIDFuelMachine, world, x, y, z);
    	return true;
    }
	...
	public TileEntity createNewTileEntity(World var1, int var2) {
		return new TileEntityFuelMachine();
	}
	...
	public static void updateBlockState(boolean isMashing, World world, int xCoord, int yCoord, int zCoord) {
		...
		if (isMashing) {
			world.setBlock(xCoord, yCoord, zCoord, CreacionDeMaquinas.blockFuelMachineActive);
		}else{
			world.setBlock(xCoord, yCoord, zCoord, CreacionDeMaquinas.blockFuelMachineIdle);
