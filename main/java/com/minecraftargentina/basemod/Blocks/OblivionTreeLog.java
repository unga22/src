...
public class OblivionTreeLog extends Block {
    private IIcon topIcon, deadIcon;

    public OblivionTreeLog(Material par2Material) {
        super(par2Material);
        setHardness(2.0F);
        setStepSound(soundTypeWood);
        setBlockName("obliviontreelog");
    ...
    public void updateTick(World world, int x, int y, int z, Random random) {
        ...
        if (meta == 4) {
            ...
            if (world.isAirBlock(x, y + 1, z)) {
                world.setBlock(x, y + 1, z, CreacionDeBloques.OblivionTreeLeaves, 1, 2);
            }
            if (world.isAirBlock(x + 1, y, z)) {
                world.setBlock(x + 1, y, z, CreacionDeBloques.OblivionTreeLeaves, 1, 2);
            }
            if (world.isAirBlock(x - 1, y, z)) {
                world.setBlock(x - 1, y, z, CreacionDeBloques.OblivionTreeLeaves, 1, 2);
            }
            if (world.isAirBlock(x, y, z + 1)) {
                world.setBlock(x, y, z + 1, CreacionDeBloques.OblivionTreeLeaves, 1, 2);
            }
            if (world.isAirBlock(x, y, z - 1)) {
                world.setBlock(x, y, z - 1, CreacionDeBloques.OblivionTreeLeaves, 1, 2);
            }
    ...
    public void registerBlockIcons(IIconRegister iconRegister) {
        this.topIcon = iconRegister.registerIcon(BaseMod.modid + ":oblivionlog_top");
        this.blockIcon = iconRegister.registerIcon(BaseMod.modid + ":oblivionlog_side");
        this.deadIcon = iconRegister.registerIcon(BaseMod.modid + ":oblivionlog_dead");
    }
