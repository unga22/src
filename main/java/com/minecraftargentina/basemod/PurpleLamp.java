...
public class PurpleLamp extends Block {
	...
	public void onBlockAdded(World world, int x, int y, int z) {
		...
		if (!world.isRemote) {
			if (this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
				...
				world.setBlock(x, y, z, BaseMod.blockPurpleLampOn, 0, 2);
			}
		}
	}
	...
	public void updateTick(World world, int x, int y, int z, Random random) {
		...
		if (!world.isRemote && this.isOn && !world.isBlockIndirectlyGettingPowered(x, y, z)) {
			world.setBlock(x, y, z, BaseMod.blockPurpleLampOff, 0, 2);
		}
	}

	public Item getItemDropped(int i, Random random, int j) {
		return Item.getItemFromBlock(BaseMod.blockPurpleLampOff);
	}

	protected ItemStack createStackedBlock(int i) {
		return new ItemStack(BaseMod.blockPurpleLampOff);
	}
